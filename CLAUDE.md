...
**Within the same context:** import the module, use `module.ClassName`. This makes the origin file obvious at every usage site.

```python
# Good — clear that JunctionWebhookPayload lives in request_schemas
...
**From another context:** import the class directly. Don't reach into another context's internal schema files (`request_schemas.py`, `dtos.py`, etc.) — those are implementation details of that context. The owning context should re-export via `schemas/__init__.py`.

```python
# Good — treats the other context's schemas as a public API
