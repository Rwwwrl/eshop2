...
from rabbitmq_topology.entities import DELAYED_RETRY_QUEUES, EXCHANGES
...
def get_delayed_retry_queue_name(queue: RabbitQueue) -> str:
    try:
        ...
        raise ValueError(
            f"No delayed-retry queue defined for '{queue.name}'. Add it to rabbitmq_topology/entities.py"
        ) from exc
